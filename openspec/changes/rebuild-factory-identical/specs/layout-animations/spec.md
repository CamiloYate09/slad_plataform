...
Las cards MUST tener un hover sutil que eleva y agrega sombra, con el borde aclarándose al hover.

#### Scenario: Feature card hover refinado
- **WHEN** el usuario hace hover sobre una card
- **THEN** la card se eleva `translateY(-4px)`
- **AND** aparece un `box-shadow` con sombra profunda sobre negro
