...
Las tres consecuencias con impacto en produccion, en orden:
...
- [ ] 1.3 Extender `robots.txt` con políticas para GPTBot, OAI-SearchBot, ClaudeBot, PerplexityBot, Google-Extended, Applebot-Extended — **NO IMPLEMENTADO (auditoria 8 ago 2026)**: `robots.txt` solo tiene `User-agent: *` + `Allow: /` + `Sitemap:`. Ningun bot especifico
- [ ] 1.4 Agregar `<script type="application/ld+json">` con `FAQPage` (mínimo 4 preguntas frecuentes — 5 incluidas) — **NO IMPLEMENTADO (auditoria 8 ago 2026)**: no hay JSON-LD `FAQPage` en `index.html`
- [ ] 1.5 Agregar JSON-LD `ItemList` de `Place` para las 5 ciudades del carousel — **NO IMPLEMENTADO (auditoria 8 ago 2026)**: no hay `ItemList` ni `Place` en `index.html`
- [ ] 1.6 Extender `Organization` schema con `address`, `areaServed: CO`, `foundingLocation: Colombia`, contact habeas-data — **PARCIAL (auditoria 8 ago 2026)**: `Organization` existe con `ContactPoint` → `citystreamCO@pm.me`, pero sin `address`, `areaServed` ni `foundingLocation`, y ese correo no es el canal habeas-data del spec
- [ ] 1.7 Agregar `WebSite` schema con `potentialAction: SearchAction` (cuando exista busqueda) o dejar como TBD — dejado TBD (sin busqueda interna por ahora), `inLanguage: es-CO` agregado — **PARCIAL (auditoria 8 ago 2026)**: `WebSite` existe con `name`/`url`; falta `inLanguage: es-CO`
...
- [ ] 2.5 Agregar `<script type="speculationrules">` con `prerender` para `/privacidad.html` y `/terminos.html` (las anclas no aplican; ajustado a rutas reales que se crean en Fase 2) — **NO IMPLEMENTADO (auditoria 8 ago 2026)**: no hay `<script type="speculationrules">` en `index.html`
- [ ] 2.6 Auditar `preload`/`fetchpriority`: solo el logo hero tiene `fetchpriority="high"` (preload + img attr) — **PARCIAL (auditoria 8 ago 2026)**: los 2 `rel=preload` son las fuentes; ningun `fetchpriority` en `index.html`
...
- [ ] 2.10 Actualizar CSP `font-src`/`connect-src` no requiere cambios aun (font self-host pendiente). Agregado `Permissions-Policy` meta best-effort — **NO IMPLEMENTADO (auditoria 8 ago 2026)**: no hay meta `Permissions-Policy` en `index.html`
...
- [ ] 4.3 Estilos compartidos: CSS `.legal-page` / `.legal-doc` agregado a `style.css` con overrides light-theme; ambas paginas usan el mismo `style.css` y CSP propia — **NO IMPLEMENTADO (auditoria 8 ago 2026)**: `privacidad.html` y `terminos.html` usan `.legal-page`, `.legal-doc`, `.legal-header`, `.legal-meta`, `.legal-footer-note` y `style.css` **no define ninguna**. Las dos paginas legales se sirven sin estilos de layout. Es el bug con impacto visible mas directo de esta auditoria
...
- [ ] 14.3 `<link rel="manifest" href="/manifest.webmanifest">` en head — **NO IMPLEMENTADO (auditoria 8 ago 2026)**: no hay `<link rel="manifest">` en `index.html` — **el manifest existe pero el sitio no lo enlaza, asi que la PWA no es instalable**
...
- [ ] 14.5 SW registration en main.js: gated por `'serviceWorker' in navigator` + `https:` (dev local skipea con info log) — **NO IMPLEMENTADO (auditoria 8 ago 2026)**: `main.js` no menciona `serviceWorker` — **el SW nunca se registra, asi que `sw.js` no corre en produccion**
